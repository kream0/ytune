...
- **Autoplay suggestions:** when the queue is about to run out, YTune adds YouTube's suggestions ("Up next") for the song that's playing, 10 at a time, skipping anything already in the queue, long videos and shorts. If YouTube has too few, it uses the song's Mix. It keeps going as long as you listen, in both `STREAM` and `STREAM + DL` mode. Suggestions show under *Suggested · autoplay* in the queue. Songs you add go before them, and *All at once* downloading only saves suggestions as they come up. Switch it off in *Settings → Playback* or at the top of the queue; turning it off removes the suggestions that haven't played yet. Tapping a search result plays just that song, not the rest of the results.
